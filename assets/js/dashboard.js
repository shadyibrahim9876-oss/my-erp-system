...
const MODULE_NAV_DATA = {
    'scm': [
        { title: "Procurement & Sourcing", items: ["Supplier Selection", "RFQ / RFP Management", "Purchase Orders", "Contract Management", "Vendor Evaluation", "Cost Analysis"] },
        { title: "Logistics & Transportation", items: ["Inbound Logistics", "Outbound Logistics", "Fleet Management", "Route Optimization", "Shipment Tracking", "Freight Management"] },
        { title: "Warehousing & Inventory", items: ["Warehouse Operations", "Stock Receiving", "Put-away", "Picking & Packing", "Cycle Counting", "Inventory Reconciliation", "Safety Stock Management"] },
        { title: "Production Planning", items: ["MRP (Material Requirements)", "Production Scheduling", "Capacity Planning", "BOM (Bill of Materials)", "Work Orders", "Product Lifecycle Mgmt"] },
        { title: "Demand Planning", items: ["Market Demand Analysis", "Statistical Forecasting", "Sales Forecast Integration", "Supply vs Demand Balancing", "Seasonal Trend Analysis"] },
        { title: "Supplier Management", items: ["Supplier Registration", "Supplier Risk Assessment", "Supplier Scorecards", "Performance Monitoring", "Compliance & Audits"] },
        { title: "Order Management", items: ["Order Creation", "Order Tracking", "Order Fulfillment", "Backorder Management", "Customer Communication"] },
        { title: "Supply Chain Analytics", items: ["KPI Dashboard", "Cost Optimization", "Lead Time Analysis", "Supply Chain Modeling", "Risk Prediction"] },
        { title: "Returns & Reverse Logistics", items: ["Return Requests", "Return Authorization", "Repair Processing", "Refurbishment", "Waste & Recycling Handling"] }
    ]
};
...
const ModuleSubHeader = () => {
    ...
    return (
        <>
            <div className={`fixed h-12 bg-cardWhite/90 dark:bg-darkCard/90 backdrop-blur-md rounded-b-3xl border-x border-b border-white/40 dark:border-gray-800 shadow-sm flex items-center px-4 transition-all duration-500 cubic-bezier(0.4, 0, 0.2, 1) z-subheader ${sidebarOpen ? 'left-[288px]' : 'left-[80px]'} right-4 ${showSubHeader ? 'top-20 opacity-100 translate-y-0' : 'top-0 opacity-0 -translate-y-full pointer-events-none'}`}>
                <div className="flex items-center justify-between w-full h-full overflow-x-auto custom-scroll px-1 gap-2">
                    {navData.map((category, index) => (
                        <button key={index} onClick={(e) => handleCategoryClick(e, index)} className={`shrink-0 px-3 py-1.5 rounded-lg text-sm font-semibold flex items-center gap-2 transition-all whitespace-nowrap hover:bg-gray-100 dark:hover:bg-gray-700 ${activeDropdown === index ? 'bg-gray-100 dark:bg-gray-700 text-accentPrimary' : 'text-textSecondary dark:text-darkTextSecondary'}`}>
                            {category.title}
            ...
            {activeDropdown !== null && navData[activeDropdown] && (
                <div className="fixed bg-white dark:bg-darkCard border border-gray-100 dark:border-gray-700 rounded-xl shadow-xl p-2 dropdown-animate z-dropdown max-h-[60vh] overflow-y-auto custom-scroll" style={dropdownStyle} onClick={(e) => e.stopPropagation()}>
                    <div className="flex flex-col gap-1">
                        {navData[activeDropdown].items.map((item, idx) => (
                            <button key={idx} onClick={() => { notify('info', item, 'Opening section...'); setActiveDropdown(null); }} className="text-left px-3 py-2 text-sm text-textPrimary dark:text-white hover:bg-bgBody dark:hover:bg-gray-800 rounded-lg transition-colors font-medium flex items-center justify-between group whitespace-nowrap">
                                {item}
            ...
            <div className="flex-1 overflow-y-auto custom-scroll px-6 pb-6">
                ...
                {activePage !== 'home' && (
                    <div className="mb-6 animate-[fadeIn_0.3s_ease-out]">
                        <button onClick={goBack} className="group flex items-center gap-3 transition-transform active:scale-95">
